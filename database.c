...
void init_default_items(item_database* db) {
    ...
    itemdb_create_location(db, "Берег бурной реки", "Вы очнулись на холодной гальке. Шумная вода омывает ваши ноги, а спину ломит от ушибов. Вокруг — непроглядная стена хвойного леса. Отсюда видны два пути: узкая тропка, уходящая в чащу на восток, и звериная тропа, петляющая вдоль реки на запад.\n Пора выбираться.",2, LOC_EMPTY, 0);
